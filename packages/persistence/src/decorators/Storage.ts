...
import type { Bucket } from "../core/bucket/Bucket";
...
const BUILT_IN_MIGRATION_STRATEGIES = {
	...
	overwrite: (newValue: unknown, _cachedValue: unknown) => newValue,
	keep: (_newValue: unknown, cachedValue: unknown) => cachedValue,
};
		...
		afterInstantiation<T extends Record<MemberKey, unknown>>(
			...
			const [get, set] = (() => {
				if (isSignal) {
					...
					(instance as Record<symbol, unknown>)[storageSymbol] = initialValue;

					const baseGetter = () => {
						return (instance as Record<symbol, unknown>)[storageSymbol];
					};
					const baseSetter = (newValue: unknown) => {
						(instance as Record<symbol, unknown>)[storageSymbol] = newValue;
					};
