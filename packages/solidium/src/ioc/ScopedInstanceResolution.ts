...
import { getOwner, onCleanup, Owner, runWithOwner } from 'solid-js';
    ...
    implements InstanceResolution
{
    ...
    saveInstance<T, Owner>(options: SaveInstanceOptions<T, Owner>): void {
        ...
        runWithOwner(owner, () => {
            onCleanup(() => {
                this.invokeInstancePreDestroy(
                    wrapper.instance as AnyInstanceType
                );
    ...
    getInstance<T, Owner>(
        options: GetInstanceOptions<T, Owner>
    ): T | undefined {
        const solidOwner = this.getParentSolidOwner(options.identifier);
        if (!solidOwner) {
            return;
    ...
    private invokeInstancePreDestroy(instance: AnyInstanceType) {
        ...
        const preDestroyMethods = classMetadata.getMethods(
