...
class MockServer {
    ...
    private port: number;
    private mockDefinitions: Map<string, MockConfig> = new Map();
     ...
     */
    async loadMocks(directory: string): Promise<void> {
        try {
            ...
            if (mockFiles.length === 0) {
                console.warn(
                    chalk.yellow(`No mock files found in ${directory}`)
                );
                return;
            }

            console.log(
                chalk.blue(`Loading ${mockFiles.length} mock files...`)
            );
            ...
            for (const file of mockFiles) {
                try {
                    ...
                    if (!Array.isArray(mockConfigs)) {
                        console.warn(
                            chalk.yellow(
                                `Invalid mock format in ${file} - expected an array of MockConfig objects`
                            )
                        );
                        continue;
                    }
                    ...
                    mockConfigs.forEach(mockConfig =>
                        this.registerMock(mockConfig, file)
                    );
                } catch (err) {
                    console.error(
                        chalk.red(`Failed to load mock file ${file}:`),
                        err
                    );
                }
            }
    ...
    registerMock(mockConfig: MockConfig, sourceFile?: string): void {
        ...
        if (this.mockDefinitions.has(routeKey)) {
            console.warn(
                chalk.yellow(`Warning: Duplicate mock route ${routeKey}`)
            );
        }
        ...
        console.log(
            chalk.green(
                `Registered: ${routeKey}${description ? ' - ' + description : ''}`
            )
        );
        if (sourceFile) {
            console.log(
    ...
    start(): void {
        ...
        this.app.listen(this.port, () => {
            ...
            console.log(
                chalk.blue(
                    `Registered ${this.mockDefinitions.size} mock endpoints:\n`
                )
            );

            // Display all registered routes
            Array.from(this.mockDefinitions.entries()).forEach(
                ([route, config]) => {
