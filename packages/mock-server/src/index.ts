...
class MockServer {
		...
		try {
			...
			for (const file of mockFiles) {
				try {
					...
					const mockConfigs: MockConfig[] = mockModule.default || mockModule;
	...
	registerMock(mockConfig: MockConfig, sourceFile?: string): void {
		...
		if (sourceFile) {
			console.log(
				chalk.gray(`  Source: ${path.relative(process.cwd(), sourceFile)}`),
			);
		}
	}
	...
	start(): void {
		...
		this.app.listen(this.port, () => {
			...
			console.log(
				chalk.blue(`Registered ${this.mockDefinitions.size} mock endpoints:\n`),
			);
